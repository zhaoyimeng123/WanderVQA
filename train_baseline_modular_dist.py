...
def main(config):
    ...
    if config.database == 'LSVQ':
        ...
        trainset = VideoDataset_images_with_LP_motion_dist_features(videos_dir, feature_dir, lp_dir, datainfo_train,
                                                                    transformations_train, 'LSVQ_train',
                                                                    config.crop_size,
                                                                    'Fast', dist_dir)

        testset = VideoDataset_images_with_LP_motion_dist_features(videos_dir, feature_dir, lp_dir, datainfo_test,
                                                                   transformations_test, 'LSVQ_test', config.crop_size,
                                                                   'Fast', dist_dir)

        testset_1080p = VideoDataset_images_with_LP_motion_dist_features(videos_dir, feature_dir, lp_dir,
                                                                         datainfo_test_1080p,
    ...
    best_test_b, best_test_s, best_test_t, best_test_d, best_test_std = [], [], [], [], []
    best_test_b_1080p, best_test_s_1080p, best_test_t_1080p, best_test_d_1080p, best_test_std_1080p = [], [], [], [], []
    ...
    for epoch in range(config.epochs):
        ...
        for i, (video, feature_3D, mos, lp, dist, _) in enumerate(train_loader):
        ...
        with torch.no_grad():
            ...
            for i, (video, feature_3D, mos, lp, dist, _) in enumerate(test_loader_1080p):
                video = video.to(device)
                feature_3D = feature_3D.to(device)
                lp = lp.to(device)
                dist = dist.to(device)
                label_1080p[i] = mos.item()
                outputs_b_1080p, outputs_s_1080p, outputs_t_1080p, outputs_d_1080p, outputs_std_1080p = model(video,
                                                                                                              feature_3D,
                                                                                                              lp, dist)
