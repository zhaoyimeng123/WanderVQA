...
def main(config):
    ...
    if config.database == 'LiveVQC':
        videos_dir = '/data/dataset/LIVE_VQC/Video'
        ...
        trainset = VideoDataset_NR_SlowFast_feature(config.database, videos_dir, datainfo, transformations, resize, config.num_frame)
    elif config.database == 'xgc_train':
        videos_dir = '/data/dataset/XGC-dataset/Share/cvprw-datasets/train/train'
        datainfo = '/data/dataset/XGC-dataset/Share/cvprw-datasets/train/train.txt'
        trainset = VideoDataset_NR_SlowFast_feature(config.database, videos_dir, datainfo, transformations, resize,
                                                    config.num_frame)
    elif config.database == 'xgc_val':
        videos_dir = '/data/dataset/XGC-dataset/DATA/DATA3/yl/data/cvprw_dataset/dataset_final/val'
        datainfo = '/data/dataset/XGC-dataset/DATA/DATA3/yl/data/cvprw_dataset/dataset_final/val.txt'
        trainset = VideoDataset_NR_SlowFast_feature(config.database, videos_dir, datainfo, transformations, resize,
                                                    config.num_frame)

    elif config.database == 'CVD2014':
        videos_dir = '/data/dataset/CVD2014'
    ...
    with torch.no_grad():
        ...
        for i, (video, mos, video_name) in enumerate(train_loader):
            ...
            print(video_name)
            if config.database == 'KoNViD-1k' or config.database == 'LiveVQC' or config.database == 'CVD2014' or config.database == 'youtube_ugc' or config.database == 'xgc':
...
if __name__ == '__main__':
    ...
    parser.add_argument('--database', type=str, default='xgc_val')
    parser.add_argument('--model_name', type=str)
    parser.add_argument('--num_workers', type=int, default=0)
    parser.add_argument('--resize', type=int, default=224)
    parser.add_argument('--num_frame', type=int, default=32)
    parser.add_argument('--gpu_ids', type=list, default=None)
    parser.add_argument('--feature_save_folder', type=str, default='/data/user/zhaoyimeng/ModularBVQA/data/xgc_ugc_slowfast/')
