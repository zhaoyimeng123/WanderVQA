...
def main(config):
    ...
    if config.database == 'LBVD':
        ...
        datainfo_test = '/data/user/zhaoyimeng/ModularBVQA/data/LBVD_data.mat'

        transformations_test = transforms.Compose([transforms.Resize([resize, resize]), transforms.ToTensor(),
                                                   transforms.Normalize(mean=[0.45, 0.45, 0.45],
                                                                        std=[0.225, 0.225, 0.225])])

        trainset = VideoDataset_NR_SlowFast2_feature_LBVD(videos_dir, datainfo_test, transformations_test, resize,
                                                          'LBVD')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--feature_save_folder', type=str,
                        default='/data/user/zhaoyimeng/ModularBVQA/data/LBVD_slowfast/')
