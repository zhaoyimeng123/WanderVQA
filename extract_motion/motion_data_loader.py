...
class VideoDataset_NR_SlowFast_feature(data.Dataset):
    ...
    def __init__(self, database_name, data_dir, filename_path, transform, resize, num_frame):
        ...
        if database_name == 'LIVEHFR':
            ...
            # 将结果分别存储到 self.video_names 和 self.score
            self.video_names = dataInfo['file_names']
            self.score = dataInfo['MOS']

        elif database_name == 'xgc_train':
            ...
            video_names = dataInfo['file_names']
            self.video_names = video_names
            self.score = pd.Series([0] * len(video_names))
    ...
    def __getitem__(self, idx):
        ...
        if self.database_name == 'KoNViD-1k' or self.database_name == 'LIVEYTGaming' or self.database_name[:3] == 'xgc':
            video_clip_min = 8
