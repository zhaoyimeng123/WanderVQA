...
from data_loader import VideoDataset_images_with_LP_motion_features, VideoDataset_images_with_LP_motion_dist_features, \
    VideoDataset_images_with_QCN_dist_features
...
def main(config):
    ...
    # distortion
    if config.model_name == 'ViTbCLIP_SpatialTemporalDistortion_loda_modular_dropout_v5':
        model = modular.ViTbCLIP_SpatialTemporalDistortion_LoDA_modular_dropout_v5(feat_len=8, sr=True, tr=True, dr=True,
                                                                           dropout_sp=0.1, dropout_tp=0.2, dropout_dp=0.2)
    ...
    if config.database == 'LSVQ':
        ...
        QCN_dir = '/data/user/zhaoyimeng/ModularBVQA/data/LSVQ_QCN_feat'

        trainset = VideoDataset_images_with_QCN_dist_features(videos_dir, feature_dir, QCN_dir, datainfo_train,
                                                                    transformations_train, 'LSVQ_train',
                                                                    config.crop_size,
                                                                    'Fast', dist_dir)

        testset = VideoDataset_images_with_QCN_dist_features(videos_dir, feature_dir, QCN_dir, datainfo_test,
                                                                   transformations_test, 'LSVQ_test', config.crop_size,
                                                                   'Fast', dist_dir)

        testset_1080p = VideoDataset_images_with_QCN_dist_features(videos_dir, feature_dir, QCN_dir,
                                                                         datainfo_test_1080p,
                                                                         transformations_test, 'LSVQ_test_1080p',
                                                                         config.crop_size, 'Fast', dist_dir)
    ...
    for epoch in range(config.epochs):
        ...
        for i, (video, feature_3D, mos, QCN_feat, dist, _) in enumerate(train_loader):

            video = video.to(device)
            feature_3D = feature_3D.to(device)
            QCN_feat = QCN_feat.to(device)
            dist = dist.to(device)
            labels = mos.to(device).float()

            outputs_std = model(video, feature_3D, QCN_feat, dist)
            optimizer.zero_grad()

            loss_st = criterion(labels, outputs_std)
        ...
        with torch.no_grad():
            ...
            for i, (video, feature_3D, mos, QCN_feat, dist, _) in enumerate(test_loader):
                video = video.to(device)
                feature_3D = feature_3D.to(device)
                QCN_feat = QCN_feat.to(device)
                dist = dist.to(device)
            ...
            print(
                'Epoch {} completed. The result on the STD test databaset: SRCC: {:.4f}, KRCC: {:.4f}, PLCC: {:.4f}, and RMSE: {:.4f}'.format(
                    epoch + 1,
                    test_SRCC_std, test_KRCC_std, test_PLCC_std, test_RMSE_std))

            label_1080p = np.zeros([len(testset_1080p)])
            y_output_std_1080p = np.zeros([len(testset_1080p)])

            for i, (video, feature_3D, mos, QCN_feat, dist, _) in enumerate(test_loader_1080p):
                video = video.to(device)
                feature_3D = feature_3D.to(device)
                QCN_feat = QCN_feat.to(device)
                dist = dist.to(device)
                label_1080p[i] = mos.item()
                outputs_std_1080p = model(video, feature_3D, QCN_feat, dist)
                y_output_std_1080p[i] = outputs_std_1080p.item()

            test_PLCC_std_1080p, test_SRCC_std_1080p, test_KRCC_std_1080p, test_RMSE_std_1080p = performance_fit(
                label_1080p, y_output_std_1080p)

            print(
                'Epoch {} completed. The result on the STD test_1080p databaset: SRCC: {:.4f}, KRCC: {:.4f}, PLCC: {:.4f}, and RMSE: {:.4f}'.format(
                    epoch + 1,
                    test_SRCC_std_1080p, test_KRCC_std_1080p, test_PLCC_std_1080p, test_RMSE_std_1080p))

            if test_SRCC_std > best_test_criterion:
                print("Update best model using best_test_criterion in epoch {}".format(epoch + 1))
                best_test_criterion = test_SRCC_std

                best_test_std = [test_SRCC_std, test_KRCC_std, test_PLCC_std, test_RMSE_std]
                best_test_std_1080p = [test_SRCC_std_1080p, test_KRCC_std_1080p, test_PLCC_std_1080p,
                                       test_RMSE_std_1080p]
                ...
                save_model_name = os.path.join(config.ckpt_path, config.model_name + '_' + \
                                               ...
                                               + '_epoch_%d_SRCC_%f.pth' % (epoch + 1, test_SRCC_std))
                torch.save(model.state_dict(), save_model_name)
                old_save_name = save_model_name

    print('Training completed.')
    print(
        'The best training result on the STD test dataset SRCC: {:.4f}, KRCC: {:.4f}, PLCC: {:.4f}, and RMSE: {:.4f}'.format(
            best_test_std[0], best_test_std[1], best_test_std[2], best_test_std[3]))
    print(
        'The best training result on the STD test_1080p dataset SRCC: {:.4f}, KRCC: {:.4f}, PLCC: {:.4f}, and RMSE: {:.4f}'.format(
            best_test_std_1080p[0], best_test_std_1080p[1], best_test_std_1080p[2], best_test_std_1080p[3]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, default='ViTbCLIP_SpatialTemporalDistortion_loda_modular_dropout_v5')
