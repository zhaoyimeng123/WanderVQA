...
from data_loader import kvq_VideoDataset_images_with_LP_motion_dist_aes_videomae_features_v2
...
def main(config):
    ...
    for i in range(10):
        ...
        if config.model_name == 'ViTbCLIP_SpatialTemporalDistortion_loda_modular_dropout_cross_attention_videomae_v10_param_kvq_v2':
            model = modular.kvq_ViTbCLIP_SpatialTemporalDistortion_loda_modular_dropout_cross_attention_videomae_v10_param_v2(feat_len=4)
        ...
        optimizer = optim.AdamW(model.parameters(), lr=config.conv_base_lr, weight_decay=1e-3)
        ...
        flop_analyzer = FlopCountAnalysis(model, (in_video.to(device), in_dist.to(device), in_videomae.to(device), in_aes.to(device)))
        ...
        if config.database == 'KVQ':
            ...
            trainset = kvq_VideoDataset_images_with_LP_motion_dist_aes_videomae_features_v2(videos_dir,
                                                                                            datainfo,
                                                                          transformations_train, 'KVQ_train',
                                                                          config.crop_size, dist_dir, videomae_feat, aes_dir, seed)

            valset = kvq_VideoDataset_images_with_LP_motion_dist_aes_videomae_features_v2(videos_dir,
                                                                                          datainfo,
                                                                          transformations_val, 'KVQ_val',
                                                                          config.crop_size, dist_dir, videomae_feat, aes_dir, seed)
            ...
            trainset = kvq_VideoDataset_images_with_LP_motion_dist_aes_videomae_features_v2(videos_dir, datainfo_train,
                                                                                     transformations_train, 'LSVQ_train',
                                                                                     config.crop_size, dist_dir, videomae_feat, aes_dir, random)

            testset = kvq_VideoDataset_images_with_LP_motion_dist_aes_videomae_features_v2(videos_dir, datainfo_test,
        ...
        for epoch in range(config.epochs):
            ...
            for i, (video, mos, dist, videomae, _, aes) in tqdm(enumerate(train_loader)):
                ...
                outputs_stda = model(video, dist, videomae, aes)
            ...
            with torch.no_grad():
                ...
                for i, (video, mos, dist, videomae, video_name_str, aes) in tqdm(enumerate(val_loader)):
                    video = video.to(device)
                    dist = dist.to(device)
                    videomae = videomae.to(device)
                    aes = aes.to(device)
                ...
                if test_SRCC_stda > best_test_criterion:
                    wait = 0
                    new_save_model_name = os.path.join(config.ckpt_path, config.database + '_' + str(seed) + '_' + config.loss_type + '_epoch_%d_SRCC_%f.pth' % (epoch + 1, test_SRCC_stda))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default='ViTbCLIP_SpatialTemporalDistortion_loda_modular_dropout_cross_attention_videomae_v10_param_kvq_v2', type=str)
    ...
    # misc
    parser.add_argument('--ckpt_path', type=str, default='ckpts_kvq/v3_sample')
    ...
    parser.add_argument('--trained_model', type=str,
                        default='/data/user/zhaoyimeng/ModularBVQA/ckpts_kvq/v3_sample/LSVQ_1_plcc_rank_epoch_7_SRCC_0.857783.pth')
                        # default=None)
