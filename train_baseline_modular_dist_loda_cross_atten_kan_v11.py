...
def save_checkpoint(model, optimizer, scheduler, epoch, path='/data/user/zhaoyimeng/ModularBVQA/ckpts_modular/checkpoint.pth'):
    """保存模型的checkpoint"""
    state = {
...
def load_checkpoint(model, optimizer, scheduler, path='/data/user/zhaoyimeng/ModularBVQA/ckpts_modular/checkpoint.pth'):
    """加载模型的checkpoint"""
...
def main(config):
    ...
    # distortion
    if config.model_name == 'ViTbCLIP_SpatialTemporalDistortion_loda_modular_dropout_cross_attention_kan_v11':
        model = modular.ViTbCLIP_SpatialTemporalDistortion_loda_modular_dropout_cross_attention_kan_v11(feat_len=8, sr=True, tr=True, dr=True, ar=True,
    ...
    # optimizer
    new_lr = config.conv_base_lr * 2
    optimizer = optim.Adam(model.parameters(), lr=new_lr, weight_decay=0.000001)
    ...
    start_epoch = load_checkpoint(model, optimizer, scheduler, path='/data/user/zhaoyimeng/ModularBVQA/ckpts_modular/checkpoint.pth')
    ...
    for epoch in range(start_epoch, config.epochs):
        ...
        with torch.no_grad():
            ...
            if test_SRCC_stda > best_test_criterion:
                ...
                if epoch > 0 and old_save_name != None:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, default='ViTbCLIP_SpatialTemporalDistortion_loda_modular_dropout_cross_attention_kan_v11')
