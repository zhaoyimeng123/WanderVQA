...
def save_checkpoint(model, optimizer, scheduler, epoch, old_save_name, path='/data/user/zhaoyimeng/ModularBVQA/ckpts_modular/checkpoint_v12_555.pth'):
    """保存模型的checkpoint"""
    state = {
    ...
    print(f"Checkpoint saved at {path}, saved old_save_name is {old_save_name}")

def load_checkpoint(model, optimizer, scheduler, path='/data/user/zhaoyimeng/ModularBVQA/ckpts_modular/checkpoint_v12_555.pth'):
    """加载模型的checkpoint"""
    if os.path.isfile(path):
        ...
        print(f"Checkpoint loaded, resuming from epoch {epoch}, old_save_name: {old_save_name}")
...
def main(config):
    ...
    # distortion
    if config.model_name == 'ViTbCLIP_SpatialTemporalDistortion_loda_modular_dropout_cross_attention_videomae_cam_v12':
    ...
    # optimizer
    new_lr = config.conv_base_lr
    # optimizer = optim.Adam(model.parameters(), lr=config.conv_base_lr, weight_decay=0.0000001)
    optimizer = optim.Adam(model.parameters(), lr=new_lr, weight_decay=5e-6)
    ...
    start_epoch, old_save_name = load_checkpoint(model, optimizer, scheduler,
                                  path='/data/user/zhaoyimeng/ModularBVQA/ckpts_modular/checkpoint_v12_555.pth')
    ...
    for epoch in range(start_epoch, config.epochs):
        ...
        for i, (video, feature_3D, mos, lp, dist, aes, videomae, _) in enumerate(train_loader):
            ...
            video = video.to(device)  # b,8,3,224,224
            dist = dist.to(device)  # b,8,4096
            videomae = videomae.to(device)  # b,8,1408

            batch_size = video.shape[0]
            # 将 video 展平到 (b, -1) 的形状
            video_flat = video.view(batch_size, -1)  # Shape: (b, self.feat_len*3*224*224)
            dist_flat = dist.view(batch_size, -1)  # Shape: (b, self.feat_len*4096)
            videomae_flat = videomae.view(batch_size, -1)  # Shape: (b, self.feat_len*1408)
        ...
        with torch.no_grad():
            ...
            if test_SRCC_stda > best_test_criterion:
                ...
                ######################################################################
                # 当test>best_test，才测试test_1080p 节省消耗时间
                ...
                for i, (video, feature_3D, mos, lp, dist, aes, videomae, _) in enumerate(test_loader_1080p):
                    ...
                    outputs_stda_1080p = model(inputs)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, default='ViTbCLIP_SpatialTemporalDistortion_loda_modular_dropout_cross_attention_videomae_cam_v12')
    ...
    parser.add_argument('--exp_version', type=int, default=555)
