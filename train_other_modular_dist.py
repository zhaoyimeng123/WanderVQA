...
def main(config):
    ...
    all_test_SRCC_d, all_test_KRCC_d, all_test_PLCC_d, all_test_RMSE_d = [], [], [], []
    all_test_SRCC_std, all_test_KRCC_std, all_test_PLCC_std, all_test_RMSE_std = [], [], [], []
    ...
    for i in range(10):
        ...
        if config.database == 'LiveVQC':
            ...
            trainset = VideoDataset_images_with_LP_motion_dist_features(videos_dir, feature_dir, lp_dir, datainfo,
                                                                   transformations_train, 'LiveVQC_train',
                                                                   config.crop_size,
                                                                   'Fast', dist_dir, seed=seed)
            ...
            #                                                      'Fast', seed=seed)
            testset = VideoDataset_images_with_LP_motion_dist_features(videos_dir, feature_dir, lp_dir, datainfo,
        ...
        for epoch in range(config.epochs):
            ...
            for i, (video, feature_3D, mos, lp, dist, _) in tqdm(enumerate(train_loader)):
            ...
            with torch.no_grad():
                ...
                for i, (video, feature_3D, mos, lp, dist, _) in enumerate(test_loader):
                    video = video.to(device)
                    feature_3D = feature_3D.to(device)
                    lp = lp.to(device)
                    dist = dist.to(device)
        ...
        all_test_SRCC_d.append(best_test_d[0])
        all_test_KRCC_d.append(best_test_d[1])
        all_test_PLCC_d.append(best_test_d[2])
        all_test_RMSE_d.append(best_test_d[3])

        all_test_SRCC_std.append(best_test_std[0])
        all_test_KRCC_std.append(best_test_std[1])
        all_test_PLCC_std.append(best_test_std[2])
        all_test_RMSE_std.append(best_test_std[3])

    print(
        'The base median results SRCC: {:.4f}, KRCC: {:.4f}, PLCC: {:.4f}, and RMSE: {:.4f}'.format(
            np.median(all_test_SRCC_b), np.median(all_test_KRCC_b), np.median(all_test_PLCC_b),
            np.median(all_test_RMSE_b)))

    print(
        'The S median results SRCC: {:.4f}, KRCC: {:.4f}, PLCC: {:.4f}, and RMSE: {:.4f}'.format(
            np.median(all_test_SRCC_s), np.median(all_test_KRCC_s), np.median(all_test_PLCC_s),
            np.median(all_test_RMSE_s)))

    print(
        'The T median results SRCC: {:.4f}, KRCC: {:.4f}, PLCC: {:.4f}, and RMSE: {:.4f}'.format(
            np.median(all_test_SRCC_t), np.median(all_test_KRCC_t), np.median(all_test_PLCC_t),
            np.median(all_test_RMSE_t)))

    print(
        'The D median results SRCC: {:.4f}, KRCC: {:.4f}, PLCC: {:.4f}, and RMSE: {:.4f}'.format(
            np.median(all_test_SRCC_d), np.median(all_test_KRCC_d), np.median(all_test_PLCC_d),
            np.median(all_test_RMSE_d)))

    print(
        'The STD median results SRCC: {:.4f}, KRCC: {:.4f}, PLCC: {:.4f}, and RMSE: {:.4f}'.format(
            np.median(all_test_SRCC_std), np.median(all_test_KRCC_std), np.median(all_test_PLCC_std),
            np.median(all_test_RMSE_std)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default='ViTbCLIP_SpatialTemporalDistortion_modular_dropout', type=str)
    ...
    parser.add_argument('--trained_model', type=str,
                        default='/data/user/zhaoyimeng/ModularBVQA/ckpts_modular/ViTbCLIP_SpatialTemporalDistortion_modular_dropout_LSVQ_plcc_NR_vNone_epoch_29_SRCC_0.883130.pth')
