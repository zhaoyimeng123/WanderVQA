    ...
    for epoch in range(start_epoch, config.epochs):
        ...
        for i, (video, mos1, mos2, mos3, mos4, mos5, mos6, dist, videomae, _, transformed_aes) in tqdm(enumerate(train_loader)):
            ...
            if (i + 1) % (config.print_samples // config.train_batch_size) == 0:
                ...
                session_start_time = time.time()
        print("sum(batch_losses1): ", sum(batch_losses1), 'len(trainset): ', len(trainset), 'config.train_batch_size: ', config.train_batch_size)
        ...
        print('Epoch %d averaged training loss1: %.4f, loss2: %.4f, loss3: %.4f, loss4: %.4f, loss5: %.4f, loss6: %.4f, loss_total: %.4f, '
        ...
        with torch.no_grad():
            ...
            test_criterion = (test_SRCC_stda+test_PLCC_stda)/2
            ##########todo 使用loss_6即总loss作为模型选取依据##############################
            if test_criterion > best_test_criterion:
                ...
                save_model_name = os.path.join(config.ckpt_path, config.database + '_epoch_%d_avg(SRCC+PLCC)_%f.pth' % (epoch + 1, test_criterion))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, default='xgc_ViTbCLIP_SpatialTemporalDistortion_loda_modular_dropout_cross_attention_videomae_v10_param')
    ...
    # misc
    parser.add_argument('--ckpt_path', type=str, default='ckpts_xgc_aes')
    ...
    parser.add_argument('--trained_model', type=str,
                        # default='/data/user/zhaoyimeng/ModularBVQA/ckpts_xgc_aes/LSVQ_epoch_7_avg(SRCC+PLCC)_0.877286.pth')
                        default='/data/user/zhaoyimeng/ModularBVQA/ckpts_xgc_aes/KVQ_epoch_28_avg(SRCC+PLCC)_0.957897.pth')
                        # default=None)
