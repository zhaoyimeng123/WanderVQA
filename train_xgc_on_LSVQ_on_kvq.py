...
def main(config):
    ...
    for i in range(10):
        ...
        if config.database == 'xgc':
            ...
            datainfo_train = '/data/dataset/KVQ/train_data.csv'
        ...
        for epoch in range(config.epochs):
            ...
            with torch.no_grad():
                ...
                for i, (video, mos1, mos2, mos3, mos4, mos5, mos6, dist, videomae, _, transformed_aes) in tqdm(enumerate(test_loader)):
                    video = video.to(device)
                    dist = dist.to(device)
                    videomae = videomae.to(device)
                    label[i] = mos6.item()
                ...
                if test_SRCC_stda > best_test_criterion:
                    ...
                    save_model_name = os.path.join(config.ckpt_path, config.database + "_round_" + str(seed) + '_dim_6_epoch_%d_SRCC_%f.pth' % (
...
if __name__ == '__main__':
    ...
    # misc
    parser.add_argument('--ckpt_path', type=str, default='ckpts_xgc_LSVQ_kvq')
    ...
    parser.add_argument('--trained_model', type=str,
                        ...
                        # default='/data/user/zhaoyimeng/ModularBVQA/ckpts_xgc_LSVQ_kvq/finetune_on_LSVQ_KVQ_round_3_dim_2_epoch_9_SRCC_0.915500.pth')
                        # default=None)
