...
def main(config):
    ...
    for i in range(10):
        ...
        if config.model_name == 'ViTbCLIP_SpatialTemporal_modular_dropout':
            if config.database == 'Waterloo_indep':
                ...
                model = modular.ViTbCLIP_SpatialTemporal_modular_dropout(feat_len=8, sr=True, tr=True, dropout_sp=0.2,
                                                                         dropout_tp=0.2)
        ...
        if config.database == 'LiveVQC':
            ...
            trainset = VideoDataset_images_with_LP_motion_features(videos_dir, feature_dir, lp_dir, datainfo,
                                                                   ...
                                                                   'Fast', seed=seed)
            valset = VideoDataset_images_with_LP_motion_features(videos_dir, feature_dir, lp_dir, datainfo,
                                                                 transformations_test, 'KoNViD-1k_val',
                                                                 config.crop_size,
                                                                 'Fast', seed=seed)
            testset = VideoDataset_images_with_LP_motion_features(videos_dir, feature_dir, lp_dir, datainfo,
    ...
    print(
        'The ST median results SRCC: {:.4f}, KRCC: {:.4f}, PLCC: {:.4f}, and RMSE: {:.4f}'.format(
            np.median(all_test_SRCC_st), np.median(all_test_KRCC_st), np.median(all_test_PLCC_st),
            np.median(all_test_RMSE_st)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default='ViTbCLIP_SpatialTemporal_modular_dropout', type=str)
    ...
    parser.add_argument('--trained_model', type=str,
                        default='/data/user/zhaoyimeng/ModularBVQA/ckpts_modular/ViTbCLIP_SpatialTemporal_modular_dropout_LSVQ_plcc_NR_vNone_epoch_24_SRCC_0.878787.pth')
