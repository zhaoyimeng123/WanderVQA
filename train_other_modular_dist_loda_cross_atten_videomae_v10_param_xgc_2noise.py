...
def main(config):
    ...
    for i in range(10):
        ...
        if config.model_name == 'ViTbCLIP_SpatialTemporal_modular_dropout_2noise':
            model = modular.ViTbCLIP_SpatialTemporal_modular_dropout_2noise(feat_len=8, dr=True, tr=False, dropout_sp=0.2, dropout_tp=0.2)
        ...
        if config.database == 'xgc':
            ...
            transformations_train = transforms.Compose(
                ...
                [transforms.Resize(config.resize, interpolation=transforms.InterpolationMode.BICUBIC),
                 ...
                 transforms.RandomApply([transforms.GaussianBlur(kernel_size=3)], p=0.3),  # 模糊
                 transforms.ToTensor(),
                 AddGaussianNoise(mean=0., std=0.02),  # 加轻度高斯噪声
        ...
        ema_best_test_criterion = -1  # SROCC min
        best_test_stda = []

        print('Starting training:')

        old_save_name = None
        ema_old_save_name = None
        ...
        for epoch in range(config.epochs):
            ...
            for i, (video, mos1, mos2, mos3, mos4, mos5, mos6, dist, videomae, _, transformed_aes) in enumerate(train_loader):
                ...
                # outputs_stda, dis_contra_loss = model(video, videomae, dist)
                # if dis_contra_loss.size(0) > 1:
                #     dis_contra_loss = dis_contra_loss.mean()
                outputs_stda = model(video, videomae, dist)

                optimizer.zero_grad()

                loss_st = criterion(labels, outputs_stda)

                # loss = loss_st + 0.05 * dis_contra_loss
                loss = loss_st
            ...
            with torch.no_grad():
                ...
                model_ema.eval()
                ema_label = np.zeros([len(testset)])
                ema_y_output_stda = np.zeros([len(testset)])
                for i, (video, mos1, mos2, mos3, mos4, mos5, mos6, dist, videomae, _, transformed_aes) in enumerate(
                        test_loader):
                    video = video.to(device)
                    dist = dist.to(device)
                    videomae = videomae.to(device)
                    ema_label[i] = mos2.item()
                    # outputs_stda, dis_contra_loss = model_ema(video, videomae, dist)
                    outputs_stda = model_ema(video, videomae, dist)
                    ema_y_output_stda[i] = outputs_stda.item()
                ema_test_PLCC_stda, ema_test_SRCC_stda, ema_test_KRCC_stda, ema_test_RMSE_stda = performance_fit(ema_label, ema_y_output_stda)
                print(
                    'Epoch {} completed. The ema result on the STDA test databaset: SRCC: {:.4f}, KRCC: {:.4f}, PLCC: {:.4f}, and RMSE: {:.4f}'.format(
                        epoch + 1,
                        ema_test_SRCC_stda, ema_test_KRCC_stda, ema_test_PLCC_stda, ema_test_RMSE_stda))
                if ema_test_SRCC_stda > best_test_criterion:
                    print("Update ema best model using ema_best_test_criterion in epoch {}".format(epoch + 1))
                    ...
                    if epoch > 0:
                        if os.path.exists(ema_old_save_name):
                            os.remove(ema_old_save_name)

                    ema_save_model_name = os.path.join(config.ckpt_path, config.database + "_ema_round_" + str(seed) + '_dim_2_epoch_%d_SRCC_%f.pth' % (
                        epoch + 1, ema_test_SRCC_stda))
                    torch.save(model_ema.state_dict(), ema_save_model_name)
                    ema_old_save_name = ema_save_model_name
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default='ViTbCLIP_SpatialTemporal_modular_dropout_2noise', type=str)
    ...
    # misc
    parser.add_argument('--ckpt_path', type=str, default='ckpts_xgc_v2_2noise')
