...
from data_loader import kvq_VideoDataset_images_with_LP_motion_dist_aes_videomae_features_v3
...
def main(config):
    ...
    for i in range(10):
        ...
        if config.model_name == 'ViTbCLIP_SpatialTemporalDistortion_loda_modular_dropout_cross_attention_videomae_v10_param_kvq_v3':
            model = modular.kvq_ViTbCLIP_SpatialTemporalDistortion_loda_modular_dropout_cross_attention_videomae_v10_param_v3(feat_len=4)
        ...
        # optimizer
        optimizer = optim.Adam(model.parameters(), lr=config.conv_base_lr, weight_decay=1e-7)
        ...
        in_aes = torch.randn(1, 784)
        in_lp = torch.rand(1, 4, 5 * 512)
        flop_analyzer = FlopCountAnalysis(model, (in_video.to(device), in_dist.to(device), in_videomae.to(device), in_aes.to(device), in_lp.to(device)))
        ...
        if config.database == 'KVQ':
            videos_dir = '/data/user/zhaoyimeng/ModularBVQA/data/KVQ/train/kvq_image_all_fps1_clip'
            datainfo = '/data/dataset/KVQ/train_data.csv'
            lp_dir = '/data/user/zhaoyimeng/ModularBVQA/data/KVQ/train/kvq_LP_ResNet18'
            feature_dir = None
            dist_dir = '/data/user/zhaoyimeng/ModularBVQA/data/KVQ/train/kvq_dist_quality_aware_clip'
            aes_dir = '/data/user/zhaoyimeng/ModularBVQA/data/KVQ/train/kvq_aes'
            videomae_feat = '/data/user/zhaoyimeng/VideoMAEv2-master/save_feat/KVQ/train/kvq_VideoMAE_feat_clip'

            trainset = kvq_VideoDataset_images_with_LP_motion_dist_aes_videomae_features_v3(videos_dir, datainfo,
                                                                          transformations_train, 'KVQ_train',
                                                                          config.crop_size, dist_dir, videomae_feat, aes_dir, lp_dir)

            valset = kvq_VideoDataset_images_with_LP_motion_dist_aes_videomae_features_v3(videos_dir, datainfo,
                                                                          transformations_test, 'KVQ_val',
                                                                          config.crop_size, dist_dir, videomae_feat, aes_dir, lp_dir)

        elif config.database == 'LBVD':
            videos_dir = '/data/user/zhaoyimeng/ModularBVQA/data/LBVD_image_all_fps1_clip'
            datainfo = '/data/user/zhaoyimeng/ModularBVQA/data/LBVD_data.mat'
            lp_dir = '/data/user/zhaoyimeng/ModularBVQA/data/LBVD_LP_ResNet18_clip'
            dist_dir = '/data/user/zhaoyimeng/ModularBVQA/data/lbvd_dist_quality_aware_clip'
            aes_dir = '/data/user/zhaoyimeng/ModularBVQA/data/lbvd_aes'
            videomae_feat = '/data/user/zhaoyimeng/VideoMAEv2-master/save_feat/LBVD_VideoMAE_feat_clip'
            trainset = kvq_VideoDataset_images_with_LP_motion_dist_aes_videomae_features_v3(videos_dir, datainfo,
                                                                                            transformations_train,
                                                                                            'LBVD_train',
                                                                                            config.crop_size, dist_dir,
                                                                                            videomae_feat, aes_dir,
                                                                                            lp_dir)
        ...
        for epoch in range(config.epochs):
            ...
            for i, (video, mos, dist, videomae, _, aes, resized_lp) in tqdm(enumerate(train_loader)):
                video = video.to(device)
                dist = dist.to(device)
                videomae = videomae.to(device)
                aes = aes.to(device)
                resized_lp = resized_lp.to(device)
                labels = mos.to(device).float()

                outputs_stda = model(video, dist, videomae, aes, resized_lp)
            ...
            with torch.no_grad():
                ...
                for i, (video, mos, dist, videomae, video_name_str, aes, resized_lp) in tqdm(enumerate(val_loader)):
                    video = video.to(device)
                    dist = dist.to(device)
                    videomae = videomae.to(device)
                    aes = aes.to(device)
                ...
                if test_SRCC_stda > best_test_criterion:
                    new_save_model_name = os.path.join(config.ckpt_path, 'finetune_kvq_clip_aes_lp_' + str(seed) + '_' + config.loss_type + '_epoch_%d_SRCC_%f.pth' % (epoch + 1, test_SRCC_stda))
...
if __name__ == '__main__':
    ...
    # input parameters
    parser.add_argument('--database', default='KVQ', type=str)
    parser.add_argument('--model_name', default='ViTbCLIP_SpatialTemporalDistortion_loda_modular_dropout_cross_attention_videomae_v10_param_kvq_v3', type=str)
    ...
    # misc
    parser.add_argument('--ckpt_path', type=str, default='ckpts_kvq/v5')
