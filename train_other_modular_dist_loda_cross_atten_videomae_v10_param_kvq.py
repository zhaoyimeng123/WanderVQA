...
def main(config):
    ...
    for i in range(10):
        ...
        # 计算 FLOPs
        # 创建三个输入张量
        in_video = torch.randn(1, 8, 3, 224, 224)
        in_dist = torch.randn(1, 8, 4096)
        in_videomae = torch.randn(1, 8, 1408)
        ...
        for epoch in range(config.epochs):
            ...
            with torch.no_grad():
                ...
                if test_SRCC_stda > best_test_criterion:
                    new_save_model_name = os.path.join(config.ckpt_path, 'kvq_' + str(seed) + '_' + config.loss_type + '_epoch_%d_SRCC_%f.pth' % (epoch + 1, test_SRCC_stda))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default='ViTbCLIP_SpatialTemporalDistortion_loda_modular_dropout_cross_attention_videomae_v10_param_kvq', type=str)
    ...
    # misc
    parser.add_argument('--ckpt_path', type=str, default='ckpts_kvq')
